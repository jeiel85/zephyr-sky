...
class LocationService {
  Future<Position> getCurrentPosition({AppLocalizations? l10n}) async {
    ...
    if (!serviceEnabled) {
      throw Exception(l10n?.locationServiceDisabled ?? '위치 서비스가 비활성화되어 있습니다. 설정에서 활성화해 주세요.');
    }
    ...
    if (permission == LocationPermission.denied) {
      ...
      if (permission == LocationPermission.denied) {
        throw Exception(l10n?.locationPermissionDenied ?? '위치 권한이 거부되었습니다.');
      }
    }
    
    if (permission == LocationPermission.deniedForever) {
      throw Exception(l10n?.locationPermissionPermanentlyDenied ?? '위치 권한이 영구적으로 거부되었습니다. 설정에서 직접 허용해 주세요.');
    } 
