...
## [2026-04-21] 공식 정식 출시 (v1.0.0)

### 작업 내용
- 모든 핵심 기능(현재 위치 날씨, 검색, 마지막 위치 저장, 상태바 알림) 정상 동작 확인.
- 브랜드 소개 랜딩 페이지 및 파비콘 디자인 적용 완료.
- GitHub Actions CI/CD 파이프라인(테스트, 빌드, 릴리즈) 완벽 자동화.
- **정식 버전 v1.0.0 리셋 및 최종 배포.**

### 현재 상태
- **Open Weather 공식 v1.0.0 배포 성공.**
- APK/AAB 자동 빌드 시스템 및 브랜드 페이지 가동 중.
- 모든 요구사항이 반영된 완벽한 정상 상태.
